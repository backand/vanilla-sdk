...
describe('Backand SDK', () => {
  ...
  describe('backand.init', () => {
    it('should initiate backand namespace with basic auth', () => {
      ...
      backand.init({
        ...
        masterToken: '0443bc91-877e-4d3d-ba03-722c96fbaef0',
        userToken: 'dab7cd94-d752-11e6-992f-0e949b6db9c0',
        runSocket: true,
        runOffline: true,
  ...
  describe('backand.auth', () => {
    it('should accept basic auth inserted in init config, username should be testsdk@backand.com', function (done) {
    ...
    it('should return an email address with getList and an equal filter', function (done) {
      this.timeout(0);
        ...
        "filter": [
          {
            "fieldName": "email",
            "operator": "equals",
            "value": "sdk@backand.com"
          }
        ],
        "sort": []
      })
          .then(res => {
            expect(res.data[0].email).to.eql('sdk@backand.com');
            done();
          })
          .catch(err => {
            done(err);
  ...
  describe('backand.object', () => {
    ...
    describe('backand.object.action', () => {
      ...
      it('get with params', function (done) {
        this.timeout(0);
        var parameters = {param1: 'test1', param2: 'test2'};
        backand.object.action.get('items', 'params', parameters)
            .then(res => {
              expect(res.data).to.eql({
                "param1": "test1",
  ...
  describe('backand.fn', () => {
    ...
    it('should return exact parameters from get', function (done) {
      backand.fn.get('jsTest1', {param: '%1~2!3@4#5$6%7^8&9*(+)@test'})
          .then(res => {
            expect(res.data.param).to.eql('%1~2!3@4#5$6%7^8&9*(+)@test');
            done();
          })
          .catch(err => {
            done(err);
